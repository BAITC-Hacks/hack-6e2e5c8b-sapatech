...
- `contract_checks.py` проверяет исходный список кампаний до sanitizer:
  ...
  Проверка различает запрошенные 10–200 и фактически исполненные 1–200
  контактов пилота; если запрос не захвачен, не выводит его из `pilot_history`.
...
- `check_submission.py` дважды запускает официальный `make_submission.py` в
  двух отдельных временных копиях без `OPENAI_API_KEY` и сравнивает SHA-256 CSV.
  Дополнительно сообщает LF/CRLF-хеши, логический хеш строк с сохранением
  порядка/колонок и версии Python/pandas/numpy, не печатая строки клиентов.
