...
- `history`: `data/change_tariff.csv`; явный history_path имеет приоритет, иначе
  ищем сначала в текущем каталоге официального пакета, затем рядом с agent.py.
  При отсутствии — пустой DataFrame и запись причины в Agent.diagnostics.
...
остатков. Фактически исполненный пилот может содержать 1–9 клиентов после
ограничения когорты/ресурсов; это не ошибка диапазона запроса (ADR-005).
При доступной аудитории и ресурсах начальное исследование включает
...
Штатное ограничение аудитории фиксируется как предупреждение; валидатор проверяет
реально исполненные контакты и расходы, а не объявляет cap ошибкой (ADR-005).
